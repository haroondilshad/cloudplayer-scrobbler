...
function on_auth() {
    ...
    // window.close(); // Removed for sidebar
}
...
// Helper function to find the music playing tab (YTM or GPM)
// This might need adjustment if the sidebar is not context-aware of the main tab.
function find_play_tab(callback) {
    chrome.tabs.query({
        url: ["*://play.google.com/music/listen*", "*://music.youtube.com/*"]
    }, function(tabs) {
        if (tabs.length > 0) {
            // Prioritize active/audible tabs if possible, or just the first one
            let playTab = tabs.find(t => t.audible) || tabs.find(t => t.active) || tabs[0];
            callback(playTab);
        } else {
            // Handle case where no music tab is open - perhaps show a message in sidebar
            console.log("No music tab found.");
            // Potentially update sidebar UI to indicate no player is active
        }
    });
}

// Listen for updates from the service worker (e.g., song changes)
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === 'PLAYER_STATE_UPDATED') {
        ...
        render_auth_link();
        if (currentPlayer && currentPlayer.has_song) {
            render_love_button(currentPlayer);
        }
    } else if (message.type === 'SETTINGS_UPDATED') {
        currentSettings = message.settings;
        render_scrobble_link();
    } else if (message.type === 'HISTORY_SYNC_COMPLETE' || message.type === 'HISTORY_SYNC_UPDATE') {
        ...
        render_sync_info(); // Re-render to update last sync time
    }
});
